# IEEE S&P 2027 논문 수정 및 Interactive Rebuttal 계획

## 1. 목표와 제출물

이번 작업의 목표는 새로운 기여를 과장하는 것이 아니라, 현재 원고에서 불명확한 보안 보장과 기존 연구 대비 위치를 정확히 밝히고 핵심 주장을 측정 결과와 일치시키는 것이다.

Text rebuttal은 수정 계획을 약속하는 문서이므로 먼저 쓰지 않는다. 먼저 9월 3일까지 실제로 반영할 수정 사항, 보안 모델, 실험 가능성, 비용 변화를 확정하고 최소한 핵심 section의 수정 초안을 만든다. 그 결과를 근거로 8월 27일 rebuttal을 작성한다. 이 순서를 통해 rebuttal에서 약속한 내용과 최종 수정 논문이 달라지는 위험을 줄인다.
...
내부 마감은 공식 마감보다 하루 빠른 **8월 26일**과 **9월 2일**로 정한다. HotCRP의 시간대와 업로드 문제를 고려하여 공식 마감 당일 제출은 피한다. `Rebuttal_EN.md`와 `Rebuttal_KO.md`는 현재 논의를 위한 예비 초안이며, 아래 수정 설계가 확정되기 전에는 제출본으로 취급하지 않는다.
...
1. LAMP가 증명하는 정확한 relation과 certified-input 인터페이스
2. 기존 연구와 비교할 항목 및 실제 실험 가능 범위
3. 9월 3일까지 구현·측정·증명할 수 있는 결과와 할 수 없는 결과

이 단계에서는 rebuttal 문장을 다듬는 것보다 theorem statement, comparison table, evaluation 항목의 초안을 먼저 만든다.

**Gate A -- 8월 24일:** 공동저자들이 다음 사항에 동의해야 한다.

- revised Theorem 6.2가 주장하는 정확한 soundness 범위
- encoding certification의 주체, relation, 비용 및 재사용 조건
- Section 2/7에 추가할 비교 대상과 표의 항목
- 새로 수행할 실험과 수행하지 못하는 실험의 근거
- 수정 후에도 유지할 성능 주장과 철회하거나 제한할 주장

### Phase 2. 핵심 수정 초안과 rebuttal 작성

Gate A 이후 Abstract, Introduction, Construction, Security Analysis, Evaluation의 핵심 수정 문단을 먼저 작성한다. Text rebuttal은 이 실제 수정 초안에서 내용을 추출하여 작성한다.

**Gate B -- 8월 25일:** 각 rebuttal 약속이 다음 중 하나의 근거를 가져야 한다.

- 이미 작성된 수정 문단 또는 theorem
- 완료된 실험 결과
- 완성된 complexity 분석
- 명확한 한계 및 비교 불가능성 설명

근거가 없는 실험이나 증명을 rebuttal에서 약속하지 않는다. Gate B 이후 논문 수정 방향을 바꿔야 한다면 rebuttal 문구도 제출 전에 함께 변경한다.

### Phase 3. 수정 논문 통합과 제출

8월 27일 이후에는 rebuttal의 약속을 기준으로 논문 전체를 통합하고, 실험·수치·증명·서술을 검증한 뒤 revised PDF와 diff PDF를 생성한다.

## 3. 최우선 과제

### P0. Certified-input 모델과 커밋된 행렬 인코딩의 유효성
...
### 4.1 Interactive protocol과 Fiat--Shamir
...
- Freivalds baseline이 동일 backend에서 reduction 효과를 측정하기 위한 비교임을 명시하고 state of the art를 대표한다고 주장하지 않는다.
- 실행 가능한 관련 연구 artifact가 있으면 statement, security level, curve 및 환경을 맞출 수 있는지 먼저 검토한 후 최소 한 기법과 비교한다.
- 직접 비교가 불가능하면 그 이유를 기록하고, 임의의 cross-platform 실행시간 비교 대신 complexity 표를 제공한다.
- 인증 계층을 구현하거나 측정하지 않았다면 online LAMP 표에 포함하지 않고, 별도 미구현 비용 및 제한사항으로 명시한다.

실험을 다시 수행해야 하는 조건은 다음과 같다.

- 반복 횟수와 분산을 새로 보고해야 하는 경우
- encoding time이 기존 총시간에 누락되거나 잘못 포함된 경우
- setup time, CRS size 또는 peak memory를 새로 측정하는 경우
- 기존 표의 원시 데이터와 본문 수치가 일치하지 않는 경우
- 관련 연구와 동일 조건의 비교 실험이 실제로 가능한 경우

Certified-input 모델을 명시하는 것만으로는 LAMP 본체 코드를 변경하지 않으므로 기존 online 실험을 전부 다시 수행할 필요는 없다. 다만 새로운 비용 분해나 통계값을 주장하려면 해당 측정은 다시 수행한다.
...
- Abstract, Introduction 및 Conclusion에서 GPT-2 prover 개선만 강조한 문장을 verifier time과 proof-size trade-off를 포함하도록 수정한다.
- “prover commits to encoded matrices and proves”라는 표현이 인증되지 않은 arbitrary root까지 포함하는 것으로 읽히지 않도록 수정한다.
...
- 논문 수정 초안이나 완료된 분석으로 뒷받침되지 않은 약속은 넣지 않는다.
- certified-input 모델이 arbitrary root에 대한 standalone soundness를 제공하는 것처럼 표현하지 않는다.
...
| 날짜 | 작업 | 산출물 |
|---|---|---|
| 8월 22일 | certified-input 모델 및 soundness 범위 설계 | certification relation, revised theorem 초안 |
| 8월 23일 | 관련 연구 원문·artifact 조사 및 실험 항목 확정 | 비교표 초안, benchmark 가능성 판단, 측정 목록 |
| 8월 24일 | Gate A 공동저자 검토 | 확정된 수정 설계, 유지·제한할 주장 목록 |
| 8월 24--25일 | 핵심 section 수정 초안 및 가능한 분석·실험 수행 | Abstract/Construction/Security/Evaluation 수정 문단, 표와 결과 |
| 8월 25일 | Gate B 및 rebuttal 작성 | 근거가 연결된 650--700단어 초안 |
| 8월 26일 | 공동저자 최종 검토 및 text rebuttal 사전 제출 | 최종 text rebuttal |
...
- [ ] Gate A에서 논문 수정 방향에 공동저자가 합의함
- [ ] 모든 수정 약속이 theorem, 수정 문단, 표, 실험 또는 명시적 한계와 연결됨
...
- [ ] certified-input relation과 인증 주체·가정 정의
- [ ] certification과 LAMP의 composition theorem 추가
- [ ] arbitrary root에는 soundness가 적용되지 않음을 명시
- [ ] certification 비용과 online LAMP 비용 분리
- [ ] 재사용 및 amortization 조건을 과장 없이 설명
- [ ] Section 2 비교 논의 확장
...
- [ ] 각 reviewer concern을 수정 section과 연결한 표 작성
- [ ] 각 수정 항목에 근거가 되는 theorem, 표, 실험 로그 또는 문단 기록
- [ ] 처리하지 못한 요청에는 기술적 이유와 논문에 추가한 limitation 기록
- [ ] rebuttal의 각 약속이 최종 diff에서 실제로 확인되는지 검토
...
- **Rebuttal은 수정 계획의 결과물이다.** 먼저 수정 방향과 실행 가능성을 확정하고, 그 뒤 실제로 지킬 수 있는 약속만 rebuttal에 쓴다.
- **비용을 이동시킨 것을 비용 제거로 표현하지 않는다.** certification을 외부 단계로 분리하더라도 non-amortized 및 end-to-end 비용을 숨기지 않는다.
- **변경 추적성을 유지한다.** reviewer concern, rebuttal 답변, 논문 수정, 실험 근거가 서로 일치해야 한다.
