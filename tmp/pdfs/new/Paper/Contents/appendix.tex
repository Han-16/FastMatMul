...
to the packed encoded vector blocks \(\{\vect{m}_{XYZB,j}\}_{j\in[n]}\) under
\(\rt_{XYZB}\), and supplies the sampled openings and CP-Link proofs required
...
\begin{itemize}
  ...
  binding;
	  ...
	  Pedersen leaves under \(\rt_{ABC}\) and \(\rt_{XYZB}\).
  ...
  \item \(\mathsf{Bad}_{\mathsf{gap}}\): an input word farther than
...
\(\rt_{XYZB}\) fixes a unique sequence of packed vector-symbol commitments at
...
\(W_A[*,j]\) block under \(\rt_{ABC}\) and the opened
\(\widehat{\vect{x}}[j]\) symbol under \(\rt_{XYZB}\).  Thus the fold check
...
\end{aligned}
\]
The earlier estimate \(4(1-\delta)^t\), which assumes exact input codewords,
does not apply to the revised protocol.  A concrete security level must instead
instantiate \(\tau\), \(\epsilon_{\mathsf{gap}}\), and \(t\) jointly.  In
...
This proves Theorem~\ref{thm:security} for the public-coin interactive protocol.
