...
\begin{revblock}
  The \emph{relative distance} is $\delta = d/n$. The unique-decoding radius is \(\tau_\ud=\frac{1}{n}\left\lfloor\frac{d-1}{2}\right\rfloor\), within which a received word has at most one nearby codeword. We use a proximity threshold \(\tau \leq \tau_\ud\) throughout the protocol.
\end{revblock}
...
Pedersen commitments can be viewed as algebraic vector commitments over a group $\mathbb{G}$ of prime order $q$. Let $\mathsf{ck}=(G_0,\ldots,G_{n-1},H)$ be a commitment key consisting of fixed generators in $\mathbb{G}$. A Pedersen commitment is defined by the following algorithms.

\begin{itemize}
\item $\cm \gets \pedC(\ck,\vect{v};o)$:
outputs a commitment $\cm := \sum_{i=0}^{n-1}\vect{v}[i]G_i + oH \in \mathbb{G}$ for a vector $\vect{v}\in\mathbb{F}^n$ and randomness $o\sample\mathbb{F}$.
...
outputs $b=1$ if $\cm \stackrel{?}{=} \sum_{i=0}^{n-1}\vect{v}[i]G_i + oH$,
