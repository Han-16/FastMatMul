...
\subsection{SNARKs and Commit-and-Prove Proofs}
...
purpose is to link two commitments to the same value.
For a SNARK-side commitment $\Gamma$ and an external leaf handle $L$, the link
relation is
\[
  \begin{aligned}
  \mathcal{R}_{\mathsf{link}}
  =
  \{\,((\Gamma,L),(a,\rho_\Gamma,\rho_L)) :\;&
  \Gamma=\mathsf{Com}_{\mathsf{snark}}(a;\rho_\Gamma)\\
  &{}\land L=\mathsf{Com}_{\mathsf{leaf}}(a;\rho_L)\,\}.
  \end{aligned}
\]
The verifier first authenticates $L$ under the relevant Merkle root and then
checks the CP-Link proof. In our artifact, many such equal-opening claims are
batched into one QA-NIZK proof.
