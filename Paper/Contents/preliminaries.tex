...
\begin{definition}[Fold operation]\label{def:fold}
For a column vector $\vect{c} = (c_0, \ldots, c_{k-1})^\top \in \mathbb{F}^k$ and a coefficient vector $\vect{r} = (r_0, \ldots, r_{k-1}) \in \mathbb{F}^k$, we define
\[
\fold(\vect{c}, \vect{r}) \coloneqq \langle \vect{r}, \vect{c} \rangle = \sum_{i=0}^{k-1} r_i \cdot c_i.
\]
Equivalently, for the $j$-th column of a row-wise encoded matrix, $\fold(\enc(\mat{M})^{(j)}, \vect{r})$ equals the $j$-th coordinate of $\enc(\vect{r}\mat{M})$.
...
Our protocol is modular with respect to the choice of SNARK backend.
