...
A \emph{linear code} over $\mathbb{F}$ is an $\mathbb{F}$-linear map $\mathcal{C}: \mathbb{F}^k \to \mathbb{F}^n$ defined by a generator matrix $G \in \mathbb{F}^{k \times n}$, so that $\mathcal{C}(\vect{m}) = \vect{m} \cdot G$ for any message $\vect{m} \in \mathbb{F}^k$.
...
We write $\enc(\vect{m}) \coloneqq \mathcal{C}(\vect{m})$ for the encoding of a message vector.
...
The key metric is the \emph{circuit size} (number of R1CS constraints or PlonK gates) of the relation $\mathcal{R}_{\mathsf{SNARK}}$ that the backend must prove, as this dominates the prover's cost.

\paragraph{Auxiliary QA-NIZK links}
We use ``QA-NIZK'' only for the auxiliary proof that a batch of public
Pedersen-style handles satisfies a linear difference-opening relation.
The CRS fixes the handle generators and the verification parameters for that
linear relation; if these parameters are serialized, their bytes are setup/CRS
material rather than online proof bytes.
The security of this auxiliary component is captured by
$\epsilon_{\mathsf{QA}}$, and Section~\ref{sec:construction} states the exact
batch relation used by the artifact.
...
In our protocol, we use a structured variant: instead of sampling $\vect{r} \sample \mathbb{F}^k$ uniformly, we sample a single $r \sample \mathbb{F}$ and set $\vect{r} = (1, r, r^2, \ldots, r^{k-1})$.
If $\mat{D} = \mat{A}\mat{B} - \mat{C} \neq \mat{0}$, then for any non-zero column $\vect{d}_j$ of $\mat{D}$, the inner product $\langle \vect{r}, \vect{d}_j \rangle = \sum_{i=0}^{k-1} (\mat{D})_{i,j} \cdot r^i$ is a non-zero univariate polynomial in $r$ of degree at most $k-1$.
By Lemma~\ref{lem:sz}, this evaluates to zero with probability at most $(k-1)/|\mathbb{F}|$.

The Vandermonde structure offers a concrete efficiency advantage beyond mere randomness reduction.
The challenge $r$ appears in the SNARK public statement $\stmt$ (Section~\ref{sec:construction}); using a single scalar keeps $|\stmt| = \mathcal{O}(1)$ field elements, independent of $k$.
Had we used a uniformly random $\vect{r} \in \mathbb{F}^k$, the statement size would grow to $\mathcal{O}(k)$, increasing verifier time for statement-size-sensitive SNARKs (e.g., transparent SNARKs where verifier time is $\mathcal{O}(|\stmt|)$) and defeating the goal of a $k$-independent verifier cost.
The soundness bound $(k-1)/|\mathbb{F}|$ is slightly weaker than the $1/|\mathbb{F}|$ bound of a fully random $\vect{r}$, but remains negligible for any $|\mathbb{F}| \ge 2^{256}$ and practical $k$.
