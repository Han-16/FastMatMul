...
\begin{itemize}
\item $\rt \gets \merkleC(\vect{v}):$ Given a vector $\vect{v}$, construct a binary hash tree with leaves $H(\vect{v}[1]), \dots, H(\vect{v}[n])$ and output the root hash $\rt$.

\item $\pi_j \gets \merkleO(\vect{v}, j):$ Open position $j$, returning the leaf value $\vect{v}[j]$ and the authentication path $\pi_j$.

\item $b \gets \merkleV(\rt, j, \vect{v}[j], \pi_j):$ Verify that $\vect{v}[j]$ is the $j$-th leaf consistent with root $\cm$; output $b \in \{0, 1\}.$

\end{itemize}

Merkle tree commitments is \emph{position-binding}: under the collision resistance of $H$, no $\ppt$ adversary can produce a root $\rt$, an index $j \in [n]$, two distinct values $v \neq v'$, and authentication paths $\pi_j,\pi'_j$ such that
\[
    \merkleV(\rt,j,v,\pi_j)=
    \merkleV(\rt,j,v',\pi'_j)=1,
\]
...
\begin{itemize}
    \item $\cm \gets \pedC(\ck, \vect{v}; r): $ On input a vector $\mathbf{v}\in\mathbb{F}^n$ and randomness $r\sample\mathbb{F}$, output $\cm := \sum_{i=1}^{n}\mathbf{v}[i]G_i + rH \in \mathbb{G}$
    \item $\pi \gets \pedO(\mathbf{v},r)$: The opening consists of the vector and the randomness, namely $\pi=(\mathbf{v},r)$.
    \item $b \gets \pedV(\ck, \cm,\mathbf{v},r)$: Returns $b=1$ if $\cm \stackrel{?}{=} \sum_{i=1}^{n}\mathbf{v}[i]G_i + rH$. Otherwise, $b=0$
\end{itemize}
...
$\cm_2=\pedC(\ck,\vect{v}_2;r_2)$,
\[
\cm_1+\cm_2 = \pedC(\ck,\vect{v}_1+\vect{v}_2;r_1+r_2).
\]
