...
\]
The verifier starts from certified public roots $(\cm_A,\cm_B,\cm_C)$ that bind
the computation inputs and output at the cost of $\mathcal{O}(1)$ hash values.
...
them, commits to the message vectors and to their encoded leaves, and sends
these commitments before the query tuple is sampled.
...
as Groth16 witness data, opens a compact public wire handle for that witness,
and provides auxiliary Merkle/CP-link material tying the wire handle to the
authenticated external leaf handle.
...
At each sampled position, the SNARK relation recomputes
$\enc(\vect{x})_j$, $\enc(\vect{y})_j$, and $\enc(\vect{z})_j$ from the
committed message vectors and checks that these values match the opened encoded
leaves.
...
The verifier is given certified roots $(\cm_A,\cm_B,\cm_C)$.
They bind to row-wise encodings of $\mat{A},\mat{B},\mat{C}$ under the linear
code $\mathcal{C}$.
...
message vectors, encodes them, and commits to the encoded leaves
$(\cm_x,\cm_y,\cm_z)$.
...
Groth16 witness data, together with public wire handles and auxiliary Merkle
openings for compact leaf handles.

\paragraph{SNARK verification}
The verifier checks message-commitment openings for the intermediate vectors,
wire-handle openings for sampled values, sampled RS evaluations for
intermediate vectors, and the four local consistency equations at each queried
index.
These checks are arithmetized into the Groth16 relation defined in
Section~\ref{sec:construction}.
Merkle path validity and CP-link consistency over the public wire/leaf handles
are auxiliary verifier checks bound to the same transcript.
...
$\mathcal{O}(t \cdot k + E_{\mathsf{msg}}(k))$, where the first term includes
sampled folds, RS evaluations, and wire-handle openings, and $E_{\mathsf{msg}}$
is the message-opening cost.
Merkle authentication and CP-link verification add
$\mathcal{O}(t\log n+E_{\mathsf{link}}(k))$ auxiliary online work.
For the CP-linked instantiation evaluated in this paper, the in-circuit checker
is linear in $k$ for fixed $t$ and independent of $k^2$ or $k^3$.
