...
\(s\in\mathbb{F}\) and sets
  $\vect{s}=(1,s,\ldots,s^{k-1})$. The independent challenge \(\vect{s}\) is used to test the proximity of the
committed encoding of \(\mat{B}\). The vector
\(\vect{x}=\vect{r}\mat{A}\) cannot serve this purpose in general because its
distribution depends on \(\mat{A}\); for example, if \(\mat{A}=0\), then
\(\vect{x}=0\) for every challenge \(r\), independently of the validity of
the committed encoding of \(\mat{B}\).
\end{revblock}
...
\begin{revblock}
  ...
  \]
  The two folds involving \(\widehat{\mat{B}}\) have different roles:
the \(\vect{s}\)-fold establishes proximity of the committed encoding of
\(\mat{B}\), whereas the \(\vect{x}\)-fold checks the consistency of the
claimed product \(\vect{y}=\vect{x}\mat{B}\).
\end{revblock}
...
\begin{revblock}
The linearity of the code allows a linear combination of the rows of
$\mat{M}$ to be evaluated directly from its encoded columns.
In particular, for any $\vect{u}=(u_0,\ldots,u_{k-1})\in\mathbb{F}^k$,
\[
...
% $\mathcal{C}^{\langle k\rangle}$.
% \end{revblock}
% For a matrix \(\mat{M}\in\mathbb{F}^{k\times k}\), the prover row-wise encodes \(\mat{M}\)
