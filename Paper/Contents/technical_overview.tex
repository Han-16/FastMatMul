...
The prover sends only the root hashes $(\cm_A, \cm_B, \cm_C)$ to the verifier, binding the computation inputs and output at the cost of $\mathcal{O}(1)$ hash values.
...
The prover computes $\vect{x}, \vect{y}, \vect{z}$ outside the SNARK, encodes them, and commits to their codewords $(\cm_x, \cm_y, \cm_z)$.
The verifier then selects a small random subset of column indices $I \subset [n]$ with $|I| = t$, and for each $j \in I$ requests the $j$-th column of every committed codeword together with its Merkle authentication path.
...
Setting $t = \mathcal{O}(\lambda / \log(1/(1-\delta)))$ drives the soundness error below $2^{-\lambda}$.
...
\paragraph{Phase 1: Commit.}
The prover encodes $\mat{A}, \mat{B}, \mat{C}$ row-wise using the linear code $\mathcal{C}$, commits to the resulting codeword matrices via Merkle trees, and sends roots $(\cm_A, \cm_B, \cm_C)$ to the verifier.
...
The prover computes the intermediate vectors $\vect{x} = \vect{r}\mat{A}$, $\vect{y} = \vect{x}\mat{B}$, $\vect{z} = \vect{r}\mat{C}$, encodes them, and commits to the encodings $(\cm_x, \cm_y, \cm_z)$.

\paragraph{Phase 4: Query.}
The verifier samples random column indices $I \subset [n]$ and sends them to the prover.
...
The verifier checks Merkle path validity and the four local consistency equations at each queried index.
...
The verifier only needs to verify $\pi$, reducing the total verifier cost to that of a single SNARK verification.

\paragraph{Cost summary.}
The \emph{SNARK circuit size}---the key metric governing prover cost---is $\mathcal{O}(t \cdot k)$, linear in $k$ and independent of $k^2$ or $k^3$.
...
The verifier checks only the SNARK proof: $\mathcal{O}(1)$ for pairing-based backends, or $\mathcal{O}(|\stmt|)$ for transparent ones.
