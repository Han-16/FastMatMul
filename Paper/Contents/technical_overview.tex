...
as Groth16 witness data, publishes compact SNARK-side commitments bound to the
relevant witnesses by the CC-SNARK backend, and provides auxiliary
opening/linking material.  The auxiliary material links full vector witnesses to
the pre-query vector commitments and links sampled witnesses to authenticated
external leaf handles.
...
The vector-level CP-Link checks bind the vectors used by the SNARK to the
pre-query vector commitments.  At each sampled position, the SNARK relation
recomputes $\enc(\vect{x})_j$, $\enc(\vect{y})_j$, and $\enc(\vect{z})_j$ from
those vector witnesses, and checks that these values match the opened encoded
leaves through the sampled linking checks.
