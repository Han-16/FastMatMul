...
The verifier checks the code-commitment backend relation, Merkle path validity,
and the four local consistency equations at each queried index.
These checks are arithmetized into the SNARK relation
$\mathcal{R}_{\mathsf{SNARK}}$ defined in Section~\ref{sec:construction}.
The prover then produces a succinct proof $\pi$ attesting that all checks pass.
The verifier only verifies $\pi$ and any backend-specific linking material.
