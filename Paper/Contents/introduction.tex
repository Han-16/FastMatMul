...
FastMatMul is an online checking layer for the committed-input setting: the
verifier starts from certified commitments to encoded input matrices, and the
protocol checks the multiplication relation among those committed objects.
The input-commitment interface and the intermediate-vector code-commitment
interface are explicit components of the statement rather than hidden costs.
Our implementation realizes the intermediate-vector backend using pre-query
commitments, in-circuit Reed--Solomon encoding checks, Merkle membership, and
QA-NIZK/CP-linking for the sampled leaves; certified input commitments are the
handoff supplied by the surrounding committed-input system.
...
\begin{itemize}
    \item \textbf{A code-based checking protocol with an explicit backend interface.}
    ...
    (e.g., expander-based codes~\cite{golovnev2023brakedown}) and a
	    ...
	    The theorem and the main asymptotic claim require the chosen
	    code-commitment backend to satisfy the stated binding, distance, and
	    cost assumptions.  We give the abstract interface and show how our
	    QA-linked implementation realizes it for the intermediate vectors,
	    while treating certified input commitments as the public-input
	    interface consumed by the online checker.
    ...
    We also give a concrete security-budget table explaining how the fixed
    $t=128$ benchmark configuration relates to stricter 128-bit query counts.
    ...
    \item \textbf{Concrete efficiency gains.}
	    ...
	    and evaluate the QA-linked instantiation against a direct Freivalds
	    SNARK baseline across matrix dimensions up to $k=2^{13}$.
    Key findings (Section~\ref{sec:implementation}):
    \begin{itemize}[nosep]
        \item \emph{Constraint reduction.}
	        The implemented checking-layer constraints, including the sampled
	        QA linking and in-circuit encoding checks for intermediate vectors,
	        grow as $\mathcal{O}(k)$ under the measured instantiation, versus
        ...
        Our QA-NIZK implementation $\fmQA$ outperforms Freivalds from
        $k = 256$ onward.
        ...
        is slower because the QA-NIZK implementation checks sampled
