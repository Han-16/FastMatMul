...
challenges:
\[
\stmt = \big( \rt_{ABC},\rt_{XYZ},r,I,\alpha_x,\alpha_y,\alpha_z, \revmath{\alpha_b}
  \big).
\]
...
\[
\comwit = \big( (\vect{m}_{ABC,j})_{j\in I},(\vect{m}_{XYZ,j})_{j\in I} \big),
\]
...
$\circ$ $\relation_{\mathsf{LAMP}}^{\mathsf{on}}(\stmt; \wit):$ \\
    ...
    $\qquad \qquad \stmt =
      \big(
        \rt_{ABC},\rt_{XYZ},r,I,\alpha_x,\alpha_y,\alpha_z, \revmath{\alpha_b}
      \big)$ \\
...
\[
\fold(\vect{r},\widehat{\mat{C}}[*,j]),
  \revmath{\fold(\vect{b},\widehat{\mat{B}}[*,j])}.
\]
...
\procedure[]{\protocol}{%
    ...
    \text{for each } j\in[n]: \\ 
    \quad \vect{m}_{XYZ,j} \coloneqq \big( \widehat{\vect{x}}[j]\, \|\,\widehat{\vect{y}}[j]\, \|\,\widehat{\vect{z}}[j], \revmath{\|\,\widehat{\vect{b}}[j]} \big) \in\mathbb{F}^{\revmath{4n}} \\ 
    \quad o_{XYZ,j}\sample\mathbb{F} \\ 
    \quad \cm_{XYZ,j} \gets \pedC(\ck_{XYZ},\vect{m}_{XYZ,j};o_{XYZ,j}) \\ 
    \rt_{XYZ} \gets \merkleC((\cm_{XYZ,j})_{j\in[n]};\bot) \< \sendmessageright*[1.3cm]{\rt_{XYZ}} \< \\
    ...
    \stmt \coloneqq
      \big(\rt_{ABC},\rt_{XYZ},r,I,\alpha_x,\alpha_y,\alpha_z, \revmath{\alpha_b}\big) \\
    \comwit \coloneqq
