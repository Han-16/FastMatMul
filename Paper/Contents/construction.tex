...
Table~\ref{tab:backend-boundary} makes this boundary explicit.
...
Linear-time committed-codeword backend & Backend certifies distance $\delta$ and global codeword binding & $\mathcal{O}(k)$ at constant rate & Target model for the $\mathcal{O}(tk)$ theorem; an end-to-end artifact must instantiate and measure it. \\
...
Current QA artifact & Merkle openings plus QA-NIZK linking for sampled columns & Measures local layer only & Reports fold/opening/linking costs; standalone $\codeComV$ cost is not independently implemented or measured. \\
\bottomrule
\end{tabularx}
\caption{Backend boundary for $\protocol$. The security theorem is relative to
a backend satisfying Definition~\ref{def:codecom-backend}; the current artifact
measures the multiplication-checking layer and not a closed end-to-end backend.}
...
\[
\begin{gathered}
  T_I \gets (\para,\cm_A,\cm_B,\cm_C,r,\cm_x,\cm_y,\cm_z),\\
