...
size from $\mathcal{O}(k^2)$ to
$\mathcal{O}(tk + t\log n + E_{\mathsf{cc}}(k))$, and to
$\mathcal{O}(tk)$ when the code-commitment backend verifier is linear.
...
(i)~we operate in the computational setting with committed-codeword openings
and SNARKs, yielding a \emph{non-interactive} protocol with succinct proofs;
...
Relative to zkMatrix, DualMatrix, and zkVC, our construction is modular in the
commitment layer: the transparent instantiation uses hash-based openings, while
the KZG and CPLink configurations expose the trade-off between algebraic
commitments, linking material, and setup assumptions.
...
that the circuit presented to the SNARK is only
$\mathcal{O}(tk + t\log n + E_{\mathsf{cc}}(k))$.
