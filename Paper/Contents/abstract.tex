...
the sampled query positions are derived. The SNARK circuit then checks only the
sampled codeword positions, while Merkle openings and CP-Link proofs bind the
vector witnesses and sampled circuit values to the committed values. We prove
soundness for this committed-input setting under the soundness of the SNARK
backend, the binding of the commitments, the correctness of the CP-Link checks,
...
generation time by \(7.24\times\); verification stays below \(0.01\) seconds in
the measured range.
