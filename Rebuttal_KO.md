...
이 경우 commitment cost는 `O(k)`에서 `O(k^2)`로 증가하지만, matrix multiplication relation 자체를 검증하는 데 필요한 constraints는 여전히 `O(k)`입니다. 또한 별도의 Merkle proofs와 CP-Link가 필요하지 않으므로, Groth16만으로 constant-size proof와 verifier complexity를 제공할 수 있습니다. 반면 zkMaP과 같이 circuit 외부에서 검증을 수행하는 방식은, 검증된 matrices와 circuit 내부에서 사용되는 matrices를 연결하기 위한 추가 mechanism이 필요합니다.
