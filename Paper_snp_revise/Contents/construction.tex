...
After receiving \(\rt_{ABC}\), the verifier samples \(r, \revmath{s}\sample\mathbb{F}\). 
The prover sets {\footnotesize \(\vect{r}=(1,r,r^2,\ldots,r^{k-1})\), \revmath{\vect{s}=(1,s,\ldots,s^{k-1}),
}}
and computes
{\footnotesize
\(\vect{x}=\vect{r}\mat{A},
...
\(\widehat{\vect{x}}=\enc(\vect{x}),
  \widehat{\vect{y}}=\enc(\vect{y}),
  \widehat{\vect{z}}=\enc(\vect{z}),
  \revmath{\widehat{\vect{b}}=\enc(\vect{b})}\).
}
The encoded intermediate vectors form a \(\revmath{4}\)-fold interleaved codeword.  For
...
\(\vect{m}_{XYZ,j}
  ...
  \big(
    ...
    \|\,\widehat{\vect{y}}[j]\,
    \|\,\widehat{\vect{z}}[j]\,
    \revmath{\|\,\widehat{\vect{b}}[j]}
  \big)
  \in\mathbb{F}^{\revmath{4}}\).
}
Using a Pedersen commitment key \(\ck_{XYZ}\), the prover samples
\(o_{XYZ,j}\sample\mathbb{F}\), computes
{\footnotesize
...
\rev{The prover also sends}
\revmath{\widetilde{\cm}_{\mathsf{int}}\gets
\bPi_{\cp}.\com(\para,\vect m_{\mathsf{int}};\widetilde o_{\mathsf{int}})}\rev{,
...
Only after receiving \(\rt_{XYZ}\) \rev{and} \revmath{\widetilde{\cm}_{\mathsf{int}}} does the verifier sample the query tuple
{\footnotesize \(I=(j_1,\ldots,j_t)\sample [n]^t\).}
It also samples public code-check points
{\footnotesize
\(\alpha_x,\alpha_y,\alpha_z, \revmath{\alpha_b}\sample\mathbb{F}\setminus D\),
}
where \(D\) is the
code evaluation domain. These points are used by the circuit to check that
{\footnotesize
\(\widehat{\vect{x}},\widehat{\vect{y}},\widehat{\vect{z}}, \revmath{\widehat{\vect{b}}}\)
}
are encodings of
...
$\circ$ $\relation_{\mathsf{LAMP}}^{\mathsf{on}}(\stmt; \wit):$ \\
    ...
    $\qquad \qquad \stmt =
      \big(
        \rt_{ABC},\rt_{XYZ},r, \revmath{s}, I,\alpha_x,\alpha_y,\alpha_z, \revmath{\alpha_b}
      \big)$ \\
    $\qquad \qquad \wit=(\comwit,\revmath{\bot})$ \\
    $\qquad \qquad \comwit =
      \big(
        (\vect{m}_{ABC,j})_{j\in I},
        (\vect{m}_{XYZ,j})_{j\in I}\revmath{,\vect m_{\mathsf{int}}}
      \big)$ \\
    $\qquad \qquad \revmath{\vect m_{\mathsf{int}}} =
      \big(
        \vect{x},\vect{y},\vect{z},\revmath{\vect{b}},
    \widehat{\vect{x}},\widehat{\vect{y}},\widehat{\vect{z}}, \revmath{\widehat{\vect{b}}}
      \big)$ \\[0.5ex]
    $\qquad \vect{r}\gets(1,r,\ldots,r^{k-1})$ \\[0.5ex]
    $\qquad \revmath{\vect{s}\gets(1,s,\ldots,s^{k-1})}$ \\[0.5ex]
    ...
    $\qquad \revmath{\mathsf{EncCheck}_{\mathcal{C}}
...
challenges:
\[
\stmt = \big( \rt_{ABC},\rt_{XYZ},r, \revmath{s}, I,\alpha_x,\alpha_y,\alpha_z, \revmath{\alpha_b}
  \big).
\]
...
The complete SNARK witness is
  $\wit=(\comwit,\revmath{\bot})$,
where the committed part of the witness is
\[
\comwit = \big( (\vect{m}_{ABC,j})_{j\in I},(\vect{m}_{XYZ,j})_{j\in I}\revmath{,\vect m_{\mathsf{int}}} \big),
\]
\rev{where the committed intermediate witness is}
  $\revmath{\vect m_{\mathsf{int}}}
  =
  \big(
    \vect{x},\vect{y},\vect{z},\revmath{\vect{b}},
    \widehat{\vect{x}},\widehat{\vect{y}},\widehat{\vect{z}}, \revmath{\widehat{\vect{b}}}
...
\(\wit=(\comwit,\revmath{\bot})\).  These witness commitments are not checked against
...
The first \rev{four} checks certify that
\(\widehat{\vect{x}},\widehat{\vect{y}},\widehat{\vect{z}}, \revmath{\widehat{\vect{b}}}\) are valid
encodings of the claimed intermediate vectors
\(\vect{x},\vect{y},\vect{z}, \revmath{\vect{b}}\).  The remaining checks are local equations on the
...
vector-symbol block \(\vect{m}_{XYZ,j}\), and then verifies the \rev{four} fold
...
provided by the offline relation \rev{in
Section~\ref{sec:openings-and-cplink}}.
...
Fiat--Shamir transform, by deriving \(r\revmath{, s}\) after \(\rt_{ABC}\) and deriving
\(I,\alpha_x,\alpha_y,\alpha_z\revmath{, \alpha_b}\) after \(\rt_{XYZ}\)
\rev{and} \revmath{\widetilde{\cm}_{\mathsf{int}}}.
...
For each queried position \(j\in I\), the CP-SNARK circuit checks \rev{four}
...
\[
\fold(\vect{r},\widehat{\mat{C}}[*,j]),
  \revmath{\fold(\vect{s},\widehat{\mat{B}}[*,j])}.
\]
...
\procedure[]{\protocol}{%
    ...
    \\
    \< \sendmessageleft*[1.3cm]{r\revmath{, s}} \<  r\revmath{, s} \sample \FF 
    \\
    \vect{r}\coloneqq(1,r,\ldots,r^{k-1}), 
    \revmath{\vect{s}\coloneqq(1,s,\ldots,s^{k-1})} \\
    \vect{x}\gets\vect{r}\mat{A}, \qquad \vect{y}\gets\vect{x}\mat{B}, \qquad \vect{z}\gets\vect{r}\mat{C}, \qquad \revmath{\vect{b}\gets\vect{s}\mat{B}} 
    \\ \widehat{\vect{x}},\widehat{\vect{y}},\widehat{\vect{z}}, \revmath{\widehat{\vect{b}}} \gets \enc(\vect{x}),\enc(\vect{y}),\enc(\vect{z}), \revmath{\enc(\vect{b})} \\ 
    \text{for each } j\in[n]: \\ 
    \quad \vect{m}_{XYZ,j} \coloneqq \big( \widehat{\vect{x}}[j]\, \|\,\widehat{\vect{y}}[j]\, \|\,\widehat{\vect{z}}[j] \revmath{\|\,\widehat{\vect{b}}[j]} \big) \in\mathbb{F}^{\revmath{4}} \\ 
    ...
    \revmath{\vect m_{\mathsf{int}}\coloneqq
      \big(\vect x,\vect y,\vect z,\vect b,
      \widehat{\vect x},\widehat{\vect y},\widehat{\vect z},\widehat{\vect b}\big)} \\
    \revmath{\widetilde o_{\mathsf{int}}\sample\mathbb F,\quad
      \widetilde{\cm}_{\mathsf{int}}\gets
      \bPi_{\cp}.\com(\para,\vect m_{\mathsf{int}};\widetilde o_{\mathsf{int}})}
      \< \sendmessageright*[1.3cm]{\rt_{XYZ}\revmath{,\widetilde{\cm}_{\mathsf{int}}}}
      \< \\
    \< \< I=(j_1,\ldots,j_t)\sample[n]^t 
    \\ 
    \< \sendmessageleft*[1.3cm]{I, \alpha_x, \alpha_y, \alpha_z, \revmath{\alpha_b}} \< \alpha_x,\alpha_y,\alpha_z, \revmath{\alpha_b}\sample\mathbb{F}\setminus D 
    \\
    ...
    \stmt \coloneqq
      \big(\rt_{ABC},\rt_{XYZ},r, \revmath{s}, I,\alpha_x,\alpha_y,\alpha_z, \revmath{\alpha_b}\big) \\
    \comwit \coloneqq
      \revmath{\big((\vect{m}_{ABC,j})_{j\in I},(\vect{m}_{XYZ,j})_{j\in I},\vect m_{\mathsf{int}}\big)} \\
    \wit \coloneqq \big(\comwit,\revmath{\bot}\big) \\
    ...
    \stmt_{\cp} \gets
      \big(
      \stmt,
      (\widetilde{\cm}_{T})_{T\in\mathcal{T}}\revmath{,\widetilde{\cm}_{\mathsf{int}}}
      \big) \\
...
(\pi_{\mathsf{link}}^{T})_{T\in\mathcal{T}}
   ...
   \\
    \< \< \stmt_{\cp} \gets \big( \stmt,(\widetilde{\cm}_{T})_{T\in\mathcal{T}}\revmath{,\widetilde{\cm}_{\mathsf{int}}} \big) \\
