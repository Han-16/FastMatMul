% Standard SNARK and CP-Link definitions were included in an earlier draft.
% They are omitted from the revised paper because Section~\ref{sec:security}
% states exactly the backend properties used by the theorem.
\iffalse
...
\begin{proof}
We prove the public-coin interactive protocol. The Fiat--Shamir transform used
by the implementation is discussed separately at the end and is not covered by
the theorem.
...
\(\vect{r}=(1,r,\ldots,r^{k-1})\) and
\(\vect{s}=(1,s,\ldots,s^{k-1})\), and define
\[
  \vect{x}=\vect{r}\mat{A},
  \qquad
  \vect{y}=\vect{x}\mat{B},
...
to the complete intermediate block \(\vect{m}_{XYZB}\) under
\(\cm_{XYZB}\), and supplies the sampled openings and CP-Link proofs required
...
\begin{itemize}
  ...
  SNARK-side sampled ABC blocks are not the corresponding blocks opened under
  \(\rt_{ABC}\), or the complete SNARK-side intermediate block is not the
  message committed by \(\cm_{XYZB}\);
  ...
  \item \(\mathsf{Bad}_{\mathsf{uni}}\): an input word farther than
  \(\tau\) from the interleaved code produces a structured fold within
  distance \(\tau\) of \(\mathcal{C}\).
\end{itemize}
...
\Pr[\mathsf{Bad}_{\mathsf{uni}}]
  &\le \frac{3(k-1)n}{|\mathbb F|}.
\end{aligned}
\]
...
\(\cm_{XYZB}\) fixes the complete messages and encoded views used by the
SNARK, and the sampled input values are exactly the messages opened under
\(\rt_{ABC}\).  In particular, the adversary cannot choose a different
intermediate encoding after seeing \(I\) or the code-check points without
breaking the linking layer.

\paragraph{Input proximity and extraction}
The root \(\rt_{ABC}\) fixes arbitrary words
...
code.  We use \(r\) for \(W_A,W_C\) and the independent \(s\) for \(W_B\).
By the one-parameter Reed--Solomon proximity gap of
Ben-Sasson et al.~\cite[Theorem~1.5]{bensasson2023proximity}, conditioned on
\(\neg\mathsf{Bad}_{\mathsf{uni}}\), the corresponding folded
...
to a unique interleaved codeword.  Decode those codewords row-wise to obtain
fixed matrices \(\mat{A},\mat{B},\mat{C}\).  These are the matrices bound by
the accepted committed statement.

\paragraph{Freivalds randomization}
On the event that all three input words are within distance \(\tau\), their
unique decodings \(\mat{A},\mat{B},\mat{C}\) are fixed by the input root and
do not depend on \(r\).  Suppose \(\mat{A}\mat{B}\neq\mat{C}\), and let
...
\[
  ...
  \frac{k-1}{|\mathbb{F}|}.
\]
Call this event \(\mathsf{Bad}_{\mathsf{Fr}}\).  Its bound is unconditional
over the verifier's choice of \(r\); it is union-bounded with
\(\mathsf{Bad}_{\mathsf{uni}}\), so no independence between the two events is
required.

\paragraph{Sampled relation checks}
Now also condition on \(\neg\mathsf{Bad}_{\mathsf{Fr}}\).  No vectors
...
relative distance they disagree on at least \(\delta n\) coordinates.  The
...
fewer than \((\delta-\tau)n\) positions.  For any queried coordinate \(j\),
code linearity gives
\[
  \enc(\vect{r}\mat{A})[j]
  =
  \fold(\vect{r},\enc(\mat{A})[*,j]).
\]
The sampled-opening backend ensures that the SNARK uses the opened
\(W_A[*,j]\) block under \(\rt_{ABC}\), while the full intermediate link
ensures that \(\widehat{\vect{x}}[j]\) is the symbol fixed by
\(\cm_{XYZB}\).  Thus the fold check
rejects whenever \(j\) lands in the disagreement set.  A uniformly sampled
coordinate misses this set with probability at most \(1-\delta+\tau\), and
...
+\frac{3(k-1)n}{|\mathbb F|}\\
&\quad+3(1-\tau)^t
+\frac{k-1}{|\mathbb{F}|}\\
&\quad+4(1-\delta+\tau)^t .
\end{aligned}
\]
The bound is parameterized by the proximity radius \(\tau\) and the number
\(t\) of sampled positions.  Section~\ref{sec:implementation} reports the
concrete \(t\) used in each experiment; its statistical sampling error is
obtained by substituting that value and the selected \(\tau\) into
\(3(1-\tau)^t+4(1-\delta+\tau)^t\).  The final security level also includes
the field-size terms, the encoding-check error, and the computational security
of the CP-SNARK, commitments, and CP-Link.

\paragraph{Zero knowledge}
Consider a hybrid that replaces the CP-SNARK proof by its simulator output and
then replaces the CP-Link proofs by their simulator outputs.  The hiding of the
Pedersen commitments conceals both the sampled input columns and the complete
intermediate block; the Merkle paths reveal only commitments at the sampled
positions.  The resulting view is simulatable from the public statement, and
the hybrid transitions are indistinguishable by the assumed zero-knowledge and
hiding properties.  Together with perfect completeness and the soundness bound
above, this proves Theorem~\ref{thm:security} for the public-coin interactive
protocol.

\paragraph{Fiat--Shamir variant}
The implementation derives \((r,s)\), \(I\), and backend batch challenges from
domain-separated hashes of the transcript. A formal random-oracle analysis of
this multi-round Fiat--Shamir transform is outside Theorem~\ref{thm:security}
...
\widehat{\vect{b}}\) at
\(\alpha_x,\alpha_y,\alpha_z,\alpha_b\).  A union bound gives the
\(4(n-1)/(|\mathbb{F}|-n)\) term used in
...
Our implementation uses a pairing-based quasi-adaptive link argument
...
\end{aligned}
\]
Setup samples nonzero \(k_0,\ldots,k_q,a\in\mathbb{F}\), publishes the
...
\[
 ...
 =e(\pi_{\mathsf{link}},A).
\]
Soundness follows from the quasi-adaptive linear-subspace assumption for this
...
non-interactive implementation derives these coefficients with Fiat--Shamir.
