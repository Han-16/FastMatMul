...
\end{aligned}
\]
For the rate-\(1/2\) Reed--Solomon code, setting
\(\tau=\delta/3\approx 1/6\) and \(t=512\) makes the sampling contribution
\(3(1-\tau)^t+4(1-\delta+\tau)^t\) smaller than \(2^{-133}\).
For BN254 and the evaluated dimensions, the two field-size terms and the
encoding-check term are smaller still.  The final security level also includes
the computational security of the CP-SNARK, commitments, and CP-Link.
