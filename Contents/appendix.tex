...
be the equality-link relation defined in Section~\ref{sec:prelim-cplink}.
For commitment keys \(\ck_1,\ldots,\ck_t\), commitments
\(\cm_1,\ldots,\cm_t\), message \(m\), and opening randomness
\((o_i)_{i=1}^{t}\), the relation contains exactly the pairs
\[
\begin{aligned}
...
the public commitments can be opened to one common message.  The main security
property for our protocol is link soundness.  If the CP-Link backend is
implemented as a NIZK or QA-NIZK, as in our implementation, it may additionally
satisfy witness zero knowledge~\cite{legosnark,qa-nizk}. \\
...
existence of one message \(m\) and openings \((o_i)_{i=1}^{t}\) such that every
commitment \(\cm_i\) opens to \(m\). \\

\noindent\textbf{Witness zero knowledge.}
When privacy of the linked message and opening randomness is required, we say
...
\(m\) or the openings \((o_i)_{i=1}^{t}\), beyond the public fact that the
commitments are linkable.
