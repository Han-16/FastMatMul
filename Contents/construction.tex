...
ABC commit
& \(\mathcal{O}(nk)\,\mathbb{G}+\mathcal{O}(n)\,\mathbb{F}\)
& -- \\
...
XYZ commit
& \(\mathcal{O}(n)\,\mathbb{G}+\mathcal{O}(n)\,\mathbb{F}\)
& -- \\

Merkle openings
& \(\mathcal{O}(t\log n) \mathbb{F}\)
& \(\mathcal{O}(t\log n)\mathbb{F} \) \\
...
CP-Link proofs.  The sampled matrix-column blocks and vector-symbol blocks are
private CP-SNARK witnesses; they are not serialized as public proof material.
...
We write \(\mathbb{G}\) for group operations used by Pedersen commitments and \(\mathbb{F}\) for field-level work in the commitment.  Let \(E_{\mathsf{snark}}^{\mathcal{P}}\) and \(E_{\mathsf{snark}}^{\mathcal{V}}\) denote the CP-SNARK proving and verification costs, respectively. Let \(E_{\mathsf{link},\mathcal{P}}(k,t)\) and \(E_{\mathsf{link},\mathcal{V}}(k,t)\) denote the prover and verifier costs, respectively, of the selected CP-Link construction.
...
using the notation above, the verifier cost is $E_{\mathsf{snark}}^\mathcal{V} + \mathcal{O}(t\log n) + E_{\mathsf{link}}^\mathcal{V}(k,t).$ \\
...
As a result, the main constraints of circuit is 
\(\mathcal{O}(tk)\).
...
\procedure[]{\protocol}{%
    ...
    \wit =
      ...
      \widehat{\vect{x}},\widehat{\vect{y}},\widehat{\vect{z}}
      \big) \\
    \widetilde{\cm}_{ABC} \gets
      \bPi_{\cp}.\com(\para,(\vect{m}_{ABC,j})_{j\in I};\widetilde{o}_{ABC}) \\
    \widetilde{\cm}_{XYZ} \gets
      \bPi_{\cp}.\com(\para,(\vect{m}_{XYZ,j})_{j\in I};\widetilde{o}_{XYZ}) \\
    \stmt_{\cp} \gets
      \big(
      \stmt,\widetilde{\cm}_{ABC},\widetilde{\cm}_{XYZ}
      \big) \\
    ...
    \< \< \stmt_{\cp} \gets \big( \stmt,\widetilde{\cm}_{ABC},\widetilde{\cm}_{XYZ} \big) \\
