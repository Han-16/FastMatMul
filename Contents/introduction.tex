...
where \(t\) is independent of the matrix dimension \(k\).  It also checks that
the committed encoded views of the intermediate vectors
\(\vect{x},\vect{y},\vect{z}\) are valid codewords for those vectors.  To bind
the circuit witnesses to the data fixed before the query positions are known,
...
implementation, verification consists of \(\mathcal{O}(1)\) pairings for SNARK
and CP-Link verification and \(\mathcal{O}(t\log n)\) hash operations for
Merkle openings.\\
...
\begin{itemize}
  ...
  vectors. 
% For fixed \(t\), the main circuit cost is linear in \(k\), namely
  % \(\mathcal{O}(tk)\).
