...
\begin{itemize}
  ...
  \(\widetilde{\cm}_{ABC}\) and \(\widetilde{\cm}_{XYZ}\), except with
...
Assume that the input root \(\rt_{ABC}\) is certified as above and that the
...
\begin{enumerate}[label=(\roman*)]
  ...
  For every \(\ppt\) prover \(\mathcal{P}^*\), if the certified matrices satisfy
  \(\mat{A}\mat{B}\neq\mat{C}\), then
  \[
  \begin{aligned}
  \Pr[\mathsf{Acc}]
