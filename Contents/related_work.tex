...
$\mathcal{O}(tk+E_{\mathsf{vec}}(k))$ in-circuit work plus
$\mathcal{O}(t\log n+E_{\mathsf{link}}(k))$ auxiliary opening/linking work
over encoded input commitments.
...
SNARK-friendly local relation over committed codewords and states the
...
over encoded input commitments, targets statement-sound checking rather
...
important comparison points, but their costs depend on different committed-root
generation, batching, setup, and transcript models.
