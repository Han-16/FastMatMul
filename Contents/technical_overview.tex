...
\]
where the \(i\)-th row of \(\widehat{\mat{M}}\) is
\(\enc(\mat{M}[i,*])\).  The \(j\)-th encoded column is denoted
...
First, the prover commits to the encoded matrix columns.  In the concrete
packed form used by our construction, the three encoded columns at position
