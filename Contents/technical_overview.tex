...
\[
  \rt_{ABC}
  \gets
  \merkleC(\cm_{ABC,1},\ldots,\cm_{ABC,n}; \bot).
\]
...
\(\widehat{\vect{z}}\). At a high level, the second Merkle
root binds the encoded vector symbols used by the sampled checks:
\[
  \rt_{XYZ}
  \gets
  \merkleC(\cm_{XYZ,1},\ldots,\cm_{XYZ,n}; \bot).
\]

The SNARK circuit does not verify Pedersen openings or Merkle paths internally.
Instead, it uses a commit-and-prove SNARK to commit to the sampled witness
...
\\
\text{External pedersen leaf commitments}
\\
